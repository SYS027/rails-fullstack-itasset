...
const Edit_Form = () => {
  ...
  useEffect(() => {
    fetchData("/api/v1/countries", setCountries);
    fetchData("/api/v1/states", setStates);
    fetchData("/api/v1/cities", setCities);
    fetchData(`/api/v1/vendors/${params.id}`, setVendorData);
  }, [params.id]);
  ...
  useEffect(() => {
    const fetchVendorData = async () => {
      try {
        const response = await fetch(`/api/v1/vendors/${params.id}`);
        if (!response.ok) {
